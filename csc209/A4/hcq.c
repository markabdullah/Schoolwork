...
char *print_currently_serving(Ta *ta_list) {
    ...
    if (temp == NULL) {
        count += strlen("No TAs are currently working.\r\n");
    }
    while (temp != NULL) {
        if (temp->current_student != NULL) {
            count += strlen("TA:  is serving .\r\n");
            count += strlen(temp->name);
            count += strlen(temp->current_student->name);
        } else {
            count += strlen("TA:  has no student\r\n");
            count += strlen(temp->name);
        }
        temp = temp->next;
    }
    ...
    if (ta_list == NULL) {
        pos += sprintf(message + pos, "No TAs are currently working.\r\n");
    }
    while (ta_list != NULL) {
        if (ta_list->current_student != NULL) {
            pos += sprintf(message + pos, "TA: %s is serving %s.\r\n",
                ta_list->name,
                ta_list->current_student->name);
        } else {
            pos += sprintf(message + pos, "TA: %s has no student\r\n", ta_list->name);
        }
...
char *print_full_queue(Student *stu_list) {
    int count = 0;
    Student *temp = stu_list;

    count += strlen("Full Queue\r\n");
    while (temp != NULL) {
        count += strlen("Student :\r\n");
        count += strlen(stu_list->name);
        count += strlen(stu_list->course->code);
        temp = temp->next_overall;
    }

    char *message = malloc(count + 1);
    int pos = 0;
    pos += sprintf(message + pos, "Full Queue\r\n");
    while (stu_list != NULL) {
        pos += sprintf(message + pos, "Student %s:%s\r\n", stu_list->name, stu_list->course->code);
        stu_list = stu_list->next_overall;
    }
    message[pos] = '\0';
    return message;
