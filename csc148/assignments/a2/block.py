...
class Block:
    ...
    def rectangles_to_draw(self) -> List[Tuple[Tuple[int, int, int],
        ...
        """
        drawables = []
        height = (self.size, self.size)
        if len(self.children) == 0:
            drawables.append((self.colour, self.position, height, 0))
            drawables.append((FRAME_COLOUR, self.position, height, 3))
        else:
            for i in range(len(self.children)):
                drawables.extend(self.children[i].rectangles_to_draw())
        if self.highlighted:
            drawables.append((HIGHLIGHT_COLOUR, self.position, height, 5))
        return drawables
    ...
    def swap(self, direction: int) -> None:
        ...
        if len(self.children) == 4:
            # Reposition child blocks to swap according to <direction> value
            if direction == 1:
                self.children = [
                    self.children[3],
                    self.children[2],
    ...
    def rotate(self, direction: int) -> None:
        ...
        """
        if len(self.children) == 4:
            # Rotate child blocks and their children
            if direction == 1:
                for i in range(len(self.children)):
            ...
            # Update block locations after changing child order
            self.update_block_locations(self.position, self.size)
    ...
    def smash(self) -> bool:
        ...
        if 0 < self.level < self.max_depth:
            #  Creates 4 new random children
            children = [random_init(self.level + 1, self.max_depth),
                        random_init(self.level + 1, self.max_depth),
                        random_init(self.level + 1, self.max_depth),
                        random_init(self.level + 1, self.max_depth)]
            # Complete the attributes for the newly created child blocks
    ...
    def update_block_locations(self, top_left: Tuple[int, int],
        ...
        if len(self.children) == 4:
            half = round(size / 2.0)
            self.children[0].position = (top_left[0] + half, top_left[1])
            self.children[1].position = top_left
            self.children[2].position = (top_left[0], top_left[1] + half)
            self.children[3].position = (top_left[0] + half, top_left[1] + half)
            for i in range(4):
                self.children[i]\
                    .update_block_locations(self.children[i].position, half)
    ...
    def get_selected_block(self, location: Tuple[int, int], level: int) \
        ...
        - 0 <= level <= max_depth
        """
        if level > self.level and len(self.children) == 4:
            half = round(self.size / 2)
            block_x = self.position[0]
            block_y = self.position[1]
            x = location[0]
            y = location[1]
            # Determine which quadrant the given location is in
            if (x < block_x + half) and (y < block_y + half):
                child = 1
            elif (x >= block_x + half) and (y < block_y + half):
                child = 0
            elif (x < block_x + half) and (y >= block_y + half):
                child = 2
            else:
                child = 3
            return self.children[child].get_selected_block(location, level)
        else:
            return self
    ...
    def flatten(self) -> List[List[Tuple[int, int, int]]]:
        ...
        """
        lst = []
        num_sub_blocks = 2 ** (self.max_depth - self.level)
        for _ in range(num_sub_blocks):  # Create empty 2D list of correct size
            lst.append([])

        # If self is an undivided block, fill the 2D list with its colour
        if len(self.children) == 0:
            for column in range(num_sub_blocks):
                for _ in range(num_sub_blocks):
                    lst[column].append(self.colour)
        else:  # Retrieve each childs flattened list and connect them together
            child0 = self.children[0].flatten()
            child1 = self.children[1].flatten()
            child2 = self.children[2].flatten()
            child3 = self.children[3].flatten()
            # Join together child columns to form self's flattened list
            for i in range(num_sub_blocks // 2):
                lst[i] = child1[i] + child2[i]
                lst[i + num_sub_blocks // 2] = child0[i] + child3[i]
        return lst
...
def random_init(level: int, max_depth: int) -> 'Block':
    ...
    Precondition:
        level <= max_depth
    """
    # Block to be returned can be subdivided only if level < max_depth
    if level < max_depth:
        ran = random.random()  # Randomly decide whether or not to subdivide
        if ran < math.exp(-0.25 * level):
            children = [random_init(level + 1, max_depth),
            ...
            for i in range(len(children)):
                children[i].parent = block
            return block
    colour = COLOUR_LIST[random.randint(0, 3)]
    block = Block(level, colour, [])
    block.max_depth = max_depth
    return block
