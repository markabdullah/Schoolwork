...
class PriorityQueue(Container[T]):
    ...
    def add(self, item: T) -> None:
        """Adds <item> to this PriorityQueue.
        NOTE: See the docstring for the 'remove' method for a sample doctest.
        """
        # Loops until it finds correct queue position, or reaches end of queue
        # Each loops checks if <item> has a greater priority than the item at
        # that index, then inserts it at that position
        n = 0
        while n < len(self._queue):
            if self._queue[n] < item:
                self._queue[n:n] = [item]
                return
            else:
                n += 1
