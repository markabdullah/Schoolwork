...
def tile_with_dominoes(n: int) -> List['Domino']:
    ...
    **IMPORTANT!!!**
    ...
    if n == 1:
        ...
        for index in range(len(upper_right_tiling)):
            x1 = upper_right_tiling[index].position[0][0] + 2 ** (n - 1)
            x2 = upper_right_tiling[index].position[1][0] + 2 ** (n - 1)
            y1 = upper_right_tiling[index].position[0][1]
            y2 = upper_right_tiling[index].position[1][1]
            upper_right_tiling[index].position[0] = (x1, y1)
            upper_right_tiling[index].position[1] = (x2, y2)
        for index in range(len(lower_left_tiling)):
            x1 = lower_left_tiling[index].position[0][0]
            x2 = lower_left_tiling[index].position[1][0]
            y1 = lower_left_tiling[index].position[0][1] + 2 ** (n - 1)
            y2 = lower_left_tiling[index].position[1][1] + 2 ** (n - 1)
            lower_left_tiling[index].position[0] = (x1, y1)
            lower_left_tiling[index].position[1] = (x2, y2)
        for index in range(len(lower_right_tiling)):
            x1 = lower_right_tiling[index].position[0][0] + 2 ** (n - 1)
            x2 = lower_right_tiling[index].position[1][0] + 2 ** (n - 1)
            y1 = lower_right_tiling[index].position[0][1] + 2 ** (n - 1)
            y2 = lower_right_tiling[index].position[1][1] + 2 ** (n - 1)
            lower_right_tiling[index].position[0] = (x1, y1)
            lower_right_tiling[index].position[1] = (x2, y2)

        # TODO (3)
        # Return the combined tiling for all four quadrants.
        tiles = upper_left_tiling + upper_right_tiling + lower_left_tiling \
                + lower_right_tiling
        return tiles
...
def _tile_2_by_2() -> List['Domino']:
    ...
    if num == 1:
        domino1 = Domino((0, 0), (1, 0))
        domino2 = Domino((0, 1), (1, 1))
    else:
        domino1 = Domino((0, 0), (0, 1))
        domino2 = Domino((1, 0), (1, 1))
