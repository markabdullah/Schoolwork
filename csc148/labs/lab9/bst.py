...
class BinarySearchTree:
    ...
    def height(self) -> int:
        ...
        if self.is_empty():
            return 0
        return max(self._right.height(), self._left.height()) + 1
    ...
    def rotate_right(self) -> None:
        ...
        REMEMBER*** Rotate Right --> ROOT GOES RIGHT, LEFT GOES UP
        ...
        if not self.is_empty():
            pivot = BinarySearchTree(self._root)
            pivot._left = self._left._right
            pivot._right = self._right
    ...
    def rotate_left(self) -> None:
        ...
        """
        if not self.is_empty():
            temp_left = BinarySearchTree(self._root)
            temp_left._left = self._left
            temp_left._right = self._right._left
            self._root = self._right._root
            self._right = self._right._right
            self._left = temp_left
